...
function getApiUrl() {
  return CONTEXT7_API_URL;
}

async function checkConnectivity() {
  const data = await requestJson(
    `${CONTEXT7_API_URL}/libs/search?libraryName=React&query=connectivity%20check&fast=true`,
    { timeout: 15000, headers: _headers(), retries: 1 }
  );
  if (!data || !Array.isArray(data.results)) {
    throw new Error('Context7 returned an unexpected response');
  }
