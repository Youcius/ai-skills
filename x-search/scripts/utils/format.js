...
function printSearchResult(session, format) {
  ...
  if (format === 'compact') {
    const summary = String(session.answer || '').split(/\r?\n/).map((line) => line.trim()).find((line) => line && !line.startsWith('#')) || 'no answer';
    return console.log(`${session.session_id} | ${session.provider} | ${session.sources.length} source(s) | ${summary.slice(0, 160)}`);
  }
  ...
  console.log(`- results: ${session.sources.length}\n`);
  console.log('## Provider 状态\n');
  session.provider_status.forEach((status) => console.log(`- ${status.name}: ${status.ok ? '✅' : '❌'}${status.detail ? ` — ${status.detail}` : ''}`));
  console.log(`\n${session.answer}\n`);
  console.log('## 来源\n');
...
module.exports = { dedupeSources, normalizeSourceUrl, buildSourceContext, formatSourceList, printSearchResult, printSources };
